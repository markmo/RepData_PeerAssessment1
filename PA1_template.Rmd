...
```{r}
activity <- read.csv("activity.csv", header=T, nrows=17568, colClasses=c("numeric", "character", "numeric"), na.strings="NA")
...
The mean of the total number of steps taken per day is `r steps.mean`, and the median is `r steps.median`.

## What is the average daily activity pattern?

```{r average_daily_activity}
interval.with.max.avg.steps <- avg.steps.by.interval[avg.steps.by.interval$steps == max(avg.steps.by.interval$steps), ]$interval
with(avg.steps.by.interval, plot(interval, steps, type="l"))
```

The 5-minute interval which, on average across all the days in the dataset, contains the maximum number of steps is interval: `r interval.with.max.avg.steps`.
...
The mean of the total number of steps taken per day is `r steps.mean.complete`, and the median is `r steps.median.complete`.
